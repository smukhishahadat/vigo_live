...
import static com.aliyun.svideo.base.AlivcSvideoEditParam.RATIO_MODE_ORIGINAL;
...
public class AlivcSvideoRecordActivity extends AppCompatActivity {
    ...
    private String selectedMusicUri = "", selectedMusicName = "", soundId = "";
    ...
    private void initRecord() {
        ...
        if (mInputParam != null) {
            ...
            if (!selectedMusicUri.isEmpty()){
                mVideoRecordView.setPreMusic(selectedMusicUri, selectedMusicName);
            }
        }
...
//                RecordCommon.copyRace(activity);
    ...
    private void getData() {
        ...
        if (intent.hasExtra("music_uri")){
            selectedMusicName = intent.getStringExtra("music_name");
            selectedMusicUri  = intent.getStringExtra("music_uri");
            selectedMusicUri  = intent.getStringExtra("music_uri");
            soundId  = intent.getStringExtra("music_id");
        }
    }
    ...
    protected void onResume() {
        ...
        mVideoRecordView.setCompleteListener(new AliyunSVideoRecordView.OnFinishListener() {
            ...
            public void onComplete(final String path, int duration, int ratio) {
                ...
                intent.putExtra("music_id", soundId);
                intent.putParcelableArrayListExtra("mediaInfos", infoList);
                AlivcSvideoRecordActivity.this.startActivityForResult(intent, 1);
    ...
    public void openEditor(){
        Intent intent = new Intent();
        intent.setClassName(AlivcSvideoRecordActivity.this, "com.aliyun.svideo.editor.EditorMediaActivity");
        intent.putExtra("mFrame", mInputParam.getFrame());
        intent.putExtra("mRatioMode", RATIO_MODE_ORIGINAL);
        intent.putExtra("mGop", mInputParam.getGop());
        intent.putExtra("mVideoQuality", mInputParam.getVideoQuality());
        intent.putExtra("mResolutionMode", mInputParam.getResolutionMode());
        intent.putExtra("mVideoCodec", mInputParam.getVideoCodec());
        intent.putExtra("canReplaceMusic", isUseMusic);
        intent.putExtra("hasWaterMark", true);
        intent.putParcelableArrayListExtra("mediaInfos", new ArrayList<MediaInfo>());
        AlivcSvideoRecordActivity.this.startActivityForResult(intent, 1);
    }


    public void openDute(){
        ...
        AlivcRecordInputParam recordParam = new AlivcRecordInputParam.Builder()
                .setResolutionMode(AliyunSnapVideoParam.RESOLUTION_360P)
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_PLAY) {
            ...
            mVideoRecordView.onActivityResult(requestCode, resultCode, data);
        }
    }
     ...
     * @param recordInputParam 录制输入参数
     */
    public static void startRecordWithMusic(Context context, AlivcRecordInputParam recordInputParam, String musicUri, String musicName, String soundId) {
        Intent intent = new Intent(context, AlivcSvideoRecordActivity.class);
        intent.putExtra(AlivcRecordInputParam.INTENT_KEY_RESOLUTION_MODE, recordInputParam.getResolutionMode());
        intent.putExtra(AlivcRecordInputParam.INTENT_KEY_MAX_DURATION, recordInputParam.getMaxDuration());
        intent.putExtra(AlivcRecordInputParam.INTENT_KEY_MIN_DURATION, recordInputParam.getMinDuration());
